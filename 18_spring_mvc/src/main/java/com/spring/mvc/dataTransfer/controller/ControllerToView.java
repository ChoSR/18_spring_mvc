...
public class ControllerToView {
	...
	// value에는 url주소를 명시, method에는 요청 타입을 명시한다.(method 생략시 get, method 모두 처리)
	...
	public String modelEx(Model model) {
		...
		for(int i = 1; i < 11; i++) {
			...
			memberDto.setEmail("anonymous@gamil.com");
	...
	public ModelAndView modelAndViewEx() { // return 타입은 String이 아니라 ModelAndView 클래스를 작성한다.
		...
		for(int i = 11; i < 21; i++) {
			...
			memberDto.setMemberGender("M");
			memberDto.setEmail("qwer@gamil.com");
			memberDto.setResidence("부산광역시");
			memberList.add(memberDto);
		}
		...
		mv.setViewName("dataTransfer/memberList");  // 포워딩 할 jsp파일 경로를 작성한다.
	...
	public String requestEx(HttpServletRequest request) {
		...
		for(int i = 21; i < 31; i++) {
			...
			memberDto.setEmail("abcdefg@gamil.com");
...
//		return new ResponseEntity<Object>(HttpStatus.OK);
		...
		// 2)
//		return new ResponseEntity<Object>("<h1>html 페이지를 반환합니다.</h1>",HttpStatus.OK);
		
		// 3)
//		return new ResponseEntity<Object>("<h1>html 페이지를 반환합니다.</h1>", new HttpHeaders(), HttpStatus.OK);
